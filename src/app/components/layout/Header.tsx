...
export function Header({ title, subtitle, action }: HeaderProps) {
  ...
  const notifications = [
    {
      id: 1,
      text: "Client Portal Redesign milestone completed",
      time: "2h ago",
      unread: true,
    },
    {
      id: 2,
      text: "Invoice #INV-2024 is due in 3 days",
      time: "5h ago",
      unread: true,
    },
    {
      id: 3,
      text: "New team member invited to workspace",
      time: "1d ago",
      unread: false,
    },
  ];
  ...
  return (
    <header className="h-16 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-6 flex items-center justify-between sticky top-0 z-20 shadow-sm">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors cursor-pointer text-sm"
          >
            ...
            {searchOpen ? (
              <input
                ...
                className="bg-transparent outline-none w-40 text-slate-700 placeholder:text-slate-400"
            ...
            <kbd
              className="hidden sm:inline-flex items-center gap-0.5 rounded border border-slate-200 bg-white px-1.5 text-slate-400"
          ...
          {searchOpen && searchQuery && filteredProjects.length > 0 && (
            <div className="absolute top-full mt-2 left-0 w-72 bg-white rounded-xl border border-slate-200 shadow-xl z-50 overflow-hidden">
              {filteredProjects.map((p) => (
                ...
                >
                  <div
                    className="size-8 rounded-lg bg-indigo-100 flex items-center justify-center text-indigo-600"
        ...
        <div
          ...
          className="size-9 rounded-full bg-gradient-to-br from-indigo-400 to-indigo-600 flex items-center justify-center text-white cursor-pointer hover:shadow-md hover:shadow-indigo-300/40 transition-all font-bold text-[0.7rem]"
        >
          {user?.firstName?.slice(0, 1) ?? user?.fullName?.slice(0, 1) ?? "U"}
